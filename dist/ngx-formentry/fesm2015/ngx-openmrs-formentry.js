...
class QuestionFactory {
    ...
    addHistoricalExpressions(schemaQuestion, question) {
        if (schemaQuestion.historicalExpression && schemaQuestion.historicalExpression.length > 0) {
            ...
            /** @type {?} */
            const origValue = this.historicalHelperService.evaluate(schemaQuestion.historicalExpression, this.dataSources);
...
class ObsAdapterHelper {
    ...
    toOpenMrsDateTimeString(datetime) {
        if (this.isEmpty(datetime)) {
            ...
            /** @type {?} */
            const formattedVal = moment$5(datetime).format();
            /** @type {?} */
        ...
        nodes.forEach(node => {
            if (node.control.value !== null &&
                ...
                switch (node.question.extras.type) {
                    case 'encounterDatetime':
                        /** @type {?} */
                        const dateValue = moment$6(node.control.value)
            ...
            if (node.question.extras.questionOptions.concept
                ...
                if (!this.showAppointments) {
                    ...
                    if (dataSource && locationUuid) {
                        /** @type {?} */
                        const startDate = moment$7(v).startOf('week').add(1, 'day').format('YYYY-MM-DD');
                        /** @type {?} */
                        const endDate = moment$7(v).endOf('week').subtract(1, 'day').format('YYYY-MM-DD');
...
/** @nocollapse */ EncounterPdfViewerService.ngInjectableDef = defineInjectable({ factory: function EncounterPdfViewerService_Factory() { return new EncounterPdfViewerService(inject(EncounterViewerService), inject(ObsValueAdapter)); }, token: EncounterPdfViewerService, providedIn: "root" });
